...
def woa(fitness, max_iter, n, dim, minx, maxx):
	...
	while Iter < max_iter:
		...
		Fb.append(tt)
		if(color and pp):
			print(colored("\r[At iteration "+str(Iter+1)+' the best fitness is '+str(tt)+"]",'blue',attrs=['bold']),end=endc)
		elif(pp):
			print("\r['At iteration '"+ str(Iter+1)+ "' the best fitness is '"+ str(tt)+"']",end=endc)
		Iter += 1
	if(pp):
		print("\n\n")
	tt=Fbest
	...
	foodscore=[]
	for i in range(n):
		foodscore.append(fitness(whalePopulation[i].position))
	return Fb,foodscore
